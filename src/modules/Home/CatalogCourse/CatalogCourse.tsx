      ...
      <Box className="catelog_listCourse">
        <Typography className="catelog__cateName">
          <Typography className="catelog_button">
            <ComputerIcon /> Lập trình Fron-end
          </Typography>
        </Typography>

        <Grid container spacing={2}>
          {catelogList.map((course, index) => {
            const isLastInRow = (index + 1) % 4 === 0; // Thẻ cuối cùng trong mỗi hàng (4 thẻ mỗi hàng)
            const positionClass = isLastInRow
              ? "last-card-inner"
              : "outer-card";
            return (
              <Grid item xs={12} sm={6} md={3} key={course.maKhoaHoc}>
                <div className={`card-container ${positionClass}`}>
                  ...
                  <Card
                    className={`tooltip ${index > 3 ? "tooltip-multiple" : ""}`}
                  >
